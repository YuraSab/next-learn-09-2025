...
### 8. Базова автентифікація (Email/Password, In/Out)

* **Ініціалізація Auth:** Сервіс Firebase Auth ініціалізується за допомогою `getAuth(app)` у `src/lib/firebase.ts`.
* **Управління глобальним станом:** Використовується **React Context** та **Client Components** для відстеження статусу входу.

| Функція Firebase | Опис |
| :--- | :--- |
| **`onAuthStateChanged(auth, callback)`** | **Real-time Listener** для автентифікації. Встановлює постійне з'єднання та викликає `callback` щоразу, коли статус користувача змінюється (вхід, вихід, токен оновлено). **Критично важливий** для синхронізації стану. |
| **`createUserWithEmailAndPassword()`** | Реєстрація нового користувача. **Автоматично** виконує вхід. |
| **`signInWithEmailAndPassword()`** | **Авторизація (Вхід)** існуючого користувача. Після успішного виконання автоматично оновлює стан сесії. |
| **`signOut(auth)`** | **Вихід** користувача з системи. Очищає токен сесії з локального сховища браузера. Це призводить до виклику `onAuthStateChanged` зі значенням `null`. |
* **Зберігання сесії:** Firebase Auth зберігає токен сесії (JWT) у **localStorage** браузера. `onAuthStateChanged` читає цей токен при завантаженні для відновлення сесії.
...
## 📝 Вивчення Firebase з Next.js: Захист та Middleware

### 9. Захист маршрутів через Next.js Middleware

* **Призначення:** Захист приватних маршрутів **до** того, як сторінка почне рендеритись.
* **Місцезнаходження:** Файл `middleware.ts` (або `.js`) **обов'язково** у корені проєкту.
* **Розрив між середовищами:** Next.js Middleware працює в Edge Runtime (сервер), а Firebase Auth зберігає сесію у `localStorage` (клієнт). Для з'єднання використовується **кука**.
* **Кука сесії:**
    * **Встановлення:** Використовуємо **Server Action** (`setSessionCookie`) для запису `idToken` Firebase у **`httpOnly`** куку.
    * **Читання:** Middleware читає цю `httpOnly` куку (`request.cookies.get('__session')`).
    * **`httpOnly: true`** — **критичний** параметр для безпеки, забороняє клієнтському JavaScript доступ до токена.
* **Логіка захисту:** Якщо запит відповідає захищеному маршруту (`PROTECTED_ROUTES`), а куки сесії немає, Middleware виконує `NextResponse.redirect('/signin')`.




## 📝 Вивчення Firebase з Next.js: Соціальна Автентифікація

### 10. Соціальна Автентифікація (Google/GitHub)

* **Механізм:** Використовується метод **`signInWithPopup(auth, provider)`**.
* **Клієнтська логіка:** Соціальний вхід є чисто клієнтською операцією. Після успішного отримання `userCredential`, його **`idToken`** має бути переданий на Server Action (`setSessionCookie`) для підтримки безпеки та роботи Middleware.
* **Багатофакторна автентифікація (Multi-factor Auth):** Firebase дозволяє користувачу мати **кілька методів входу** (Email, Google, GitHub) для **одного облікового запису**.
* **Обробка конфліктів:** Якщо виникає помилка `auth/account-exists-with-different-credential`, це означає, що пошта вже використовується.
* **Зв'язування (`linkWithCredential`):** Якщо користувач **вже залогінений**, ми використовуємо **`linkWithCredential(auth.currentUser, pendingCredential)`** для автоматичного додавання нового провайдера до його профілю. Це дозволяє йому входити за допомогою будь-якого з пов'язаних методів.
* **Перевага:** Єдиний механізм сесії (`httpOnly` кука) використовується для всіх методів входу.





## 📝 Вивчення Firebase з Next.js: Адміністрування та Безпека

### 11. Firebase SDK: Клієнтський VS Адмін (Архітектура та Ролі)
Firebase Admin SDK (adminDB) — це спеціалізований інструмент, розроблений для використання на безпечних бекенд-середовищах (таких як Node.js, 
де виконуються Next.js Server Actions). Він вимагає наявності приватного ключа облікового запису сервісу для ініціалізації. 
Його головна перевага — це обхід Правил Безпеки Firestore, що робить його ідеальним для операцій Автентифікації та Авторизації, які вимагають найвищого рівня довіри.
